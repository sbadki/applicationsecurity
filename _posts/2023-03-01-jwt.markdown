...
Here, securely transmitting means the data can't be tampered with, as its digitally signed. Optionally the data can also be encrypted. In simple terms if JWT is not encrypted 
but encoded its referred as JWS(Jason Web Signature) and when its encrypted its referred as (Jason Web Encryption)
...
The **header** describes the cryptographic operations applied to the JWT with other optional properties. Typically we use "Typ" - Type of the token which is always JWT, 
and "alg" - The algorithm which would be used to sign the JSON Web Token. There are various algorithm to specify but mostly common once are HS256 and RS256.
...
The **payload** represents a JSON object, it consists of claims conveyed by the JWT which is typically used by the server to verify that the user has permission to perform the 
...
If we Decode it we could see the JSON object for Header, Payload with the secret to verify the signature. Data passed in header or payload is in plain text, so it's not good 
idea to pass any sensitive information in those section.
...
Jason Web Token are short-lived so server can also issue client a refresh token with access token. Refresh token has more validity than access token like 30 days or 1 month
based on your application requirement. So that if user needs to access the resource after access token is expired instead of requesting user for authenticating himself again
server can send a refresh token to client and client can send the refresh token to server to get a new access token. Then client can continue accessing resource until the
access token is valid and so on.

### Refresh token flow ###


![jwtstructure]({{ "/assets/images/jwt/fig14-refreshtoken.png" | relative_url }})


## What are disadvantages of JWT ##

There is a dis-advantage with the access tokens or the refresh token is that if the token is stolen then attacker can make use of that token and perform malicious action until the
token gets expired. With access token it will have minimal impact as its short-lived but with refresh token it could have bigger impact. That's why it's suggested not to use tokens
for session management instead use it for Authorization purpose and use only short-lived token.

Another issue with token is that to keep track of blacklist tokens. That means will have to keep blacklist tokens in storage like database and for every API request,will have to
check against the blacklist token in db whether token is present or not. If the token is present that means is not the valid request and server can simply discard that request.
So the whole purpose of using token instead of session for session management is lost as now on we will have to make a call to DB.Thia is also the reason using token for Authorization.


## What is Jason Web Token Exploits and its Mitigation? ##
...
When "algo":"none" is specified that means token is not signed. With the help of this attacker a bypass signature by changing the value of alg to none and stripping of signature 
from the JWT.
...
If server uses weak secret to sign the JWT then it's very easy for to bruit force it and re-create the JWT with the signature. Bruit forcing for a secret is easier with the help
of hashcat utility.
...
This attack is possible when there is flaw in JWT validation, say at server end application is only checking if kid value is present. It's not checking the kid value came in JWT is
same as one present in jwks present at server end.
...
At times server refers keys from JWK set URL (jku). When server has to verify the key it fetches the key from this url. Attacker can change the jku parameter value to point to their
own JWK instead of the valid one. This allows an attacker to sign malicious token using their own private key and send it. Then the application would fetch this jwk and use it to 
...
If application is using an asymmetric encryption, it can openly publish its public key and keep the private key secret. This allows the application to sign tokens using its private 
key and anyone can verify this token using its public key.

We created malicious key using servers public key and sign the token using that malicious key. Also changed the alg to HS256.  Here the application does not check whether the 
algorithm of the received token matches the expected algorithm because of which algorithm confusion arises.
...
Github repo: https://github.com/sbadki/applicationsecurity/tree/main/springboot-jwtsecurity
