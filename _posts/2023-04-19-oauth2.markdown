...
**Implicit:** Used for browser based applications like pure Javascript Single Page Applications(SPAs)
...
2. After clicking on login the user is redirected to the Oauth Authorization server, after which the Oauth login 
   prompt is shown to the user.
    ...
    Authorization Request contains:
...
2. Client application creates a code_verifier and generates code_challenge from it. It should be cryptographically
   generated random number.

3. User is redirected to the Oauth Authorization server, along with it sends code challenge after which the Oauth login
   prompt is shown to the user.

4. User provide its credentials and authenticates based on the Oauth login options its selects. User is requested to provide
...
7. Authorization server validates code_challenge and code_verified
...
5. Resource server enquire about the Access token with Authorization server, after validating it Resource server returns user data.
...
- Authorization code flow with PKCE (this will go through in the future post)
